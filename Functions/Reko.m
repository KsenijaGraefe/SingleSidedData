...
%% SNR -> aussortieren der Frequnzkompnneten
[gutfreqX_alle,gutfreqY_alle] = SNR_Energie_Standardabweichung(SM_particle,SM_empty_1(:,:,:,1:(size(SM_particle,4))),3.65,3.65,120);
...
%% Gewichtungsamtrix(SM1H*W*SM1+lambda*I)*c=SM1H*W*u und Regularisierung
w = [];
    SH = ctranspose(S);

    for j= 1:size(S,1)
        w(j) = sqrt(sum((abs(S(j,:))).^2)); %euklidische norm, komplexweritger vektor v=(3,i,5-i) => ||v||2=sgrt(|3|^2+|i|^2+|5-i|^2)=sqrt(9+1+26)=6
    end
